(function (root, factory) {
  ...
  const statusText = (status, activityState, protocolAvailable) => status === "connected" && protocolAvailable === "false" ? "直播页面已打开，但暂时没有获取到互动数据" : status === "connected" && protocolAvailable === "unknown" ? "页面已打开 · 等待互动数据" : status === "connected" && activityState === "quiet" ? "采集正常 · 最近暂无新互动" : ({ connecting: "正在连接", authenticating: "正在连接", connected: "采集正常", stopping: "正在停止", stale: "采集可能中断", stopped: "采集已停止", error: "采集出错", offline: "采集已停止", service_offline: "本地服务没有响应", idle: "尚未开始采集" }[status] || "状态未知");
  ...
  function dashboardPresentation(input = {}) {
    ...
    else if (input.errorCode === "api_error" || input.snapshotFailed) { headline = "本地服务没有响应"; guidance = "本地服务没有响应。请点击“重新检查”；如果仍不行，请回到启动器重启服务或检查安装环境。"; }
    ...
    else if (status === "service_offline") { headline = "本地服务没有响应"; guidance = "请回到启动器重新启动服务，或运行安装环境检查。"; actions = ["retry"]; }
    ...
    else if (status === "stopped" || status === "offline" || status === "idle") { headline = status === "idle" ? "尚未开始采集" : "采集已停止"; guidance = status === "idle" ? "选择直播间并点击连接即可开始。" : "采集任务已停止，本地服务仍在运行。可以重新连接或修改直播间。"; actions = ["connect", "settings"]; }
