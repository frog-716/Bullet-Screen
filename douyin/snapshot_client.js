(function (root, factory) {
  ...
  function clearSnapshotState(previous, error) { return { ...previous, connected: false, metrics: null, events: [], lastValidAt: null, lastProtocolAt: null, protocolAvailable: "unknown", activityState: "unknown", protocolHealth: "unknown", freshness: null, lastError: String(error || "snapshot failed") }; }
  ...
  const statusText = (status, activityState, protocolAvailable) => status === "connected" && protocolAvailable === "false" ? "协议采集不可用" : status === "connected" && protocolAvailable === "unknown" ? "页面已打开 · 等待协议采集" : status === "connected" && activityState === "quiet" ? "采集正常 · 最近暂无新互动" : ({ connecting: "正在连接", authenticating: "正在验证连接", connected: "采集正常", stopping: "正在停止，暂时不能重新启动", stale: "采集可能中断", stopped: "已停止", error: "采集出错", offline: "服务离线", idle: "等待连接" }[status] || "状态未知");
