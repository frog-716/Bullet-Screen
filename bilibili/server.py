...
def verify_schema_signature(connection: sqlite3.Connection) -> None:
    ...
    if legacy_tables and legacy_tables != LEGACY_SCHEMA_TABLES:
        raise SchemaVersionError("Bilibili legacy schema is incomplete")
    for table, required in SCHEMA_COLUMNS.items():
        ...
        for name, expected in required.items():
            if name not in actual or actual[name] != expected:
                raise SchemaVersionError(f"Bilibili schema signature mismatch in {table}.{name}")
    for index_name, (table, expected_columns) in SCHEMA_INDEXES.items():
        ...
        if not index_row:
            raise SchemaVersionError(f"Bilibili schema signature missing index {index_name}")
        actual_columns = tuple(
            row[2] for row in connection.execute(f"PRAGMA index_info({index_name})")
        )
        if actual_columns != expected_columns:
            raise SchemaVersionError(f"Bilibili schema signature mismatch in index {index_name}")
    for table in ("sessions", "events", "metric_snapshots", "capture_gaps"):
        foreign_keys = {
            (row[2], row[3], row[4]) for row in connection.execute(f"PRAGMA foreign_key_list({table})")
        }
...
class EventStore:
    ...
    def _ensure_schema(self) -> None:
        ...
        if tables and version != self.SCHEMA_VERSION:
            raise SchemaVersionError(f"Bilibili database schema {version} requires an explicit migration to {self.SCHEMA_VERSION}")
        if tables and not expected_tables.issubset(tables):
            raise SchemaVersionError("Bilibili database schema is incomplete; refusing to write")
        if version > self.SCHEMA_VERSION:
            raise SchemaVersionError(f"Bilibili database schema {version} is newer than supported {self.SCHEMA_VERSION}")
        if version != 0 and not tables:
            raise SchemaVersionError(f"Bilibili database schema {version} has no matching schema; refusing to write")
        if tables:
    ...
    def recent_events(self, session_id: int, limit: Optional[int] = 100, since: Optional[str] = None) -> List[Dict[str, Any]]:
        clauses = ["session_id=?"]
    ...
    if command.startswith("SEND_GIFT"):
        ...
        event.update(
            ...
            value_contract={
                "quantity": gift_num,
                "quantity_unit": "item",
                "quantity_semantics": "platform_reported",
                "unit_price": platform_amount,
                "currency": None,
...
class Collector:
    ...
    def _handle_packet(self, context_or_payload: Any, payload: Optional[bytes] = None) -> None:
        ...
        if context is not None:
            ...
            with self.lock:
                if self.status_name not in {"connecting", "authenticating", "connected", "stale"}:
                    return
                self.last_valid_at = utc_now()
                self._last_valid_monotonic = time.monotonic()
